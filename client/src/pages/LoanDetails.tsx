    ...
    <div className="min-h-screen bg-background py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Button
          ...
          className="mb-6 pl-0 text-muted-foreground hover:bg-transparent hover:text-foreground"
        >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-muted-foreground">
                  ID: #{loan.id} • Submitted on{" "}
                  {format(new Date(loan.createdAt!), "MMMM d, yyyy")}
                </p>
              </div>
              <RiskBadge
                category={
                  loan.status === "submitted" ? "Pending Review" : loan.status
                }
                ...
                <Card className="border-primary/20 bg-primary/5">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                      <BrainCircuit className="w-5 h-5 text-primary" /> AI
                      Analysis Report
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    ...
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                        {(loan.aiScore.recommendations as string[]).map(
                          (rec, i) => (
                            <li key={i}>{rec}</li>
                          ),
                        )}
                    ...
                    <div className="bg-white p-4 rounded-lg border">
                      ...
                      <div className="space-y-2">
                        {(loan.aiScore.explanation as string[]).map(
                          (exp, i) => (
              ...
              <Card className="bg-gradient-to-br from-secondary/30 to-background border-dashed">
                <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-muted-foreground max-w-md mb-6">
                    Run our Shariah-compliant AI model to evaluate this
                    application's eligibility and risk profile.
                  </p>
                  <Button
                    onClick={handleAssess}
                    disabled={assessLoan.isPending}
                    size="lg"
                  >
            ...
            {loan.aiScore && (
              <Card className="overflow-hidden shadow-lg border-primary/20">
                ...
                <CardContent className="pt-6">
                  <h3 className="text-center font-semibold text-muted-foreground uppercase tracking-widest text-xs mb-4">
                  ...
                  <div className="space-y-3 pt-4 border-t">
                    <ScoreRow
                      label="Affordability"
                      score={loan.aiScore.affordabilityScore || 0}
                    />
                    <ScoreRow
                      label="Stability"
                      score={loan.aiScore.stabilityScore || 0}
                    />
                    <ScoreRow
                      label="Documentation"
                      score={loan.aiScore.documentationScore || 0}
                    />
...
function ScoreRow({ label, score }: { label: string; score: number }) {
  ...
  const colorClass =
    score > 80 ? "bg-primary" : score > 50 ? "bg-yellow-500" : "bg-red-500";
